...
- Don't slice the work into iterations, phases, or "what we'll build
  first." The orchestrator decides what subset of the spec to attack
  on each pass; the spec describes the desired end-state of the
  system, not the order in which it gets built. If you catch yourself
  writing "in iteration 1 we'll do X, then later Y," stop — that's
  the orchestrator's job, not the spec's. (Capturing the *current
  state of the world* is fine and different: e.g. "Google
  credentials aren't configured yet, so the test suite stands in for
  Google with a double" describes a real present condition the
  build agent has to cope with, not a phased build plan.)
